...
try {
if (!text.trim()) return conn.reply(m.chat, `✍️ *Por favor, ingresa el nombre de la música a descargar.*`, m)
await m.react('⏰')
...
if (seconds > 1800) throw '⚠️ *El video supera el límite de duración (10 minutos).*'
...
await conn.sendFile(m.chat, video.url, `${title}.mp4
await m.react('✔️')
}} catch (e) {
await m.react('✖️')
